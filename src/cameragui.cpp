            ...
            if(pBufferFilled == nullptr){
                ...
                if (this->isCameraTracking->load(std::memory_order_acquire)){ // if tracking is activated, calculate centroids with either windowing or thresholding.
                    ...
                    if (this->m_imageContainer->imgCounter < this->m_imageContainer->sampleSize ){
                        ...
                        /*int size = rand() % 10;
                        int intensity = (rand() % 200) + 10;
                        int position1row = cvimg.rows/2 - (30 + (rand() % 20));
                        int position1col = cvimg.cols/2 - (30 + (rand() % 20));
                        int position2row = cvimg.rows/2 + (30 + (rand() % 20));
                        int position2col = cvimg.cols/2 + (30 + (rand() % 20));
                        cv::circle(cvimg, cv::Point(position1row, position1col),size,cv::Scalar(intensity,0,0),-1);
                        cv::circle(cvimg, cv::Point(position2row, position2col),size,cv::Scalar(intensity,0,0),-1);
                        */
                        ///
